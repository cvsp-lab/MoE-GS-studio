...
from gaussian_renderer_emb import render, all_t_gaussians, render_frames, render_experts
import torchvision
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, cam_type):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        rendering = render(view, gaussians, pipeline, background,cam_type=cam_type, basicfunction=rbfbasefunction, cam_no=cam_no, frame_no=frame_no, iter=iteration, num_down_emb_c=emb.emb_min_embeddings, num_down_emb_f=emb.emb_min_embeddings)["render"]
...
def render_gate(model_path, name, iteration, views, gaussians, deform, pipeline, background, cam_type):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        rendering = render(view, gaussians, pipeline, background,cam_type=cam_type, basicfunction=rbfbasefunction, cam_no=cam_no, frame_no=frame_no, iter=iteration, num_down_emb_c=emb.emb_min_embeddings, num_down_emb_f=emb.emb_min_embeddings, gate_viz=True, d_xyz=d_xyz, d_rotation=d_rotation, d_scaling=d_scaling)["render"]  # pylint: disable=unexpected-keyword-arg
        render_images.append(to8b(rendering).transpose(1,2,0))
...
def render_trajectory_expert(expert_data_list, view, model_path, name, iteration, views, gaussians, pipeline, background, cam_type, N=50, step=2):
...
def render_trajectory_combined(
    expert_data_list,
    view,
    model_path,
    name,
    ...
    combined_path = os.path.join(model_path, name, f"ours_{iteration}", "combined")
    ...
    for i in range(0, T - N + 1, step):
        ...
        for j in range(N):
            ...
            for expert_name in expert_names:
                ...
                data = expert_data_list[i+j][expert_name]

                opacities_list.append(data["opacity"] * w[j])
                means3D_list.append(data["means3D"])
                means2D_list.append(data["means2D"])
                shs_list.append(data["shs"])
                scales_list.append(data["tiny_scales"])
                rotations_list.append(data["rotations"])

        opacities_cat = torch.cat(opacities_list, dim=0)
        means3D_cat = torch.cat(means3D_list, dim=0)
        means2D_cat = torch.cat(means2D_list, dim=0)
        shs_cat = torch.cat(shs_list, dim=0)
        scales_cat = torch.cat(scales_list, dim=0)
        rotations_cat = torch.cat(rotations_list, dim=0)
    ...
    imageio.mimwrite(
        os.path.join(combined_path, "combined_trajectory.mp4"),
        render_images,
        fps=15
    )

def render_expert(model_path, name, iteration, views, gaussians, pipeline, background, cam_type):
    expert_path = os.path.join(model_path, name, "ours_{}".format(iteration), "experts")
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        rendering, expert_data = render_experts(view, gaussians, pipeline, background,cam_type=cam_type, basicfunction=rbfbasefunction, cam_no=cam_no, frame_no=frame_no, iter=iteration, num_down_emb_c=emb.emb_min_embeddings, num_down_emb_f=emb.emb_min_embeddings, gate_viz=True)
        deform_image, emb_image = rendering["deform_image"], rendering["emb_image"]   

        deform_images.append(to8b(deform_image).transpose(1,2,0))
        emb_images.append(to8b(emb_image).transpose(1,2,0))
        
        expert_data_list.append(expert_data)


    imageio.mimwrite(os.path.join(expert_path, 'video_deform.mp4'), deform_images, fps=30)
    imageio.mimwrite(os.path.join(expert_path, 'video_embedding.mp4'), emb_images, fps=30)
...
def render_sets(dataset : ModelParams, hyperparam, iteration : int, pipeline : PipelineParams, embparam : ModelEmbParams, skip_train : bool, skip_test : bool, skip_video: bool):
    with torch.no_grad():
        ...
        # expert_data_list, view = render_expert(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background,cam_type)
        # render_trajectory_expert(expert_data_list, view, dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background,cam_type)
        # render_trajectory_combined(expert_data_list, view, dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background,cam_type)
