...
from scene.cameras import cameraList_from_camInfosVideo2, cameraList_from_camInfosVideo3, camera_to_JSON
...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians, load_iteration=None, shuffle=False, resolution_scales=[1.0], load_cameras=True, opt=None, use_timepad=False, render=False, save_path = None, init_mixprob=0.9):
        """b
        ...
        self.init_mixprob = init_mixprob
        ...
        if load_cameras:
            for resolution_scale in resolution_scales:
                ...
                if loader in ["colmapvalid", "colmap", "neural3dvideo"]:     
                ...
                # Technicolor
                    ...
                    self.train_cameras[resolution_scale] = cameraList_from_camInfosVideo3(scene_info.train_cameras, resolution_scale, args)
                    self.train_cameras[resolution_scale] = sorted(self.train_cameras[resolution_scale], key=lambda x: (x.timestamp, x.colmap_id))
                ...
                # Technicolor
                    ...
                    self.test_cameras[resolution_scale] = cameraList_from_camInfosVideo3(scene_info.test_cameras, resolution_scale, args)
                    self.test_cameras[resolution_scale] = sorted(self.test_cameras[resolution_scale], key=lambda x: (x.timestamp, x.colmap_id))
            ...
            if loader in ["colmapvalid", "colmap", "neural3dvideo"]:     
                ...
                for cam in self.train_cameras[resolution_scale]:
                    if cam.cam_name not in raydict and cam.rayo is not None:
                        raydict[cam.cam_name] = torch.cat([cam.rayo, cam.rayd], dim=1).cuda() # 1 x 6 x H x W
                for cam in self.test_cameras[resolution_scale]:
                    if cam.image_name not in raydict and cam.rayo is not None:
                        raydict[cam.cam_name] = torch.cat([cam.rayo, cam.rayd], dim=1).cuda() # 1 x 6 x H x W

                for cam in self.train_cameras[resolution_scale]:
                    cam.rays = raydict[cam.cam_name]
                for cam in self.test_cameras[resolution_scale]:
                    cam.rays = raydict[cam.cam_name]
    ...
    def save(self, iteration, save_path):
        ...
        point_cloud_path = os.path.join(self.model_path, "Ex4DGS/point_cloud/iteration_{}".format(iteration))
    ...
    def getTrainCameras(self, scale=1.0, shuffle=True, return_as="generator", n_job=2, return_path=False, get_img=True, job_batch_size=2):
        if self.lazy_loader:
            ...
            t_cams = list(self.train_cameras[scale])
    ...
    def getTestCameras(self, scale=1.0, shuffle=True, return_as="generator", n_job=2, return_path=False, get_img=True, job_batch_size=2):
        if self.lazy_loader:
            ...
            t_cams = list(self.test_cameras[scale])
    ...
    def set_sampling_len(self, sample_len, min_timestamp=0, sample_every=1):
        ...
        self.samplelist = [i.timestamp >= sample_len for i in self.train_cameras[1.0]]
