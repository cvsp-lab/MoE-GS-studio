...
import torch
import numpy as np
from thirdparty.reparmetrize.utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation, build_rotation_4d, build_scaling_rotation_4d
...
from thirdparty.reparmetrize.utils.graphics_utils import BasicPointCloud
from thirdparty.reparmetrize.utils.general_utils import strip_symmetric, build_scaling_rotation
from thirdparty.reparmetrize.utils.sh_utils import sh_channels_4d
import matplotlib.pyplot as plt
from helper_model import getcolormodel


class FGaussianModel:
    ...
    def __init__(self, sh_degree : int, gaussian_dim : int = 3, time_duration: list = [-0.5, 0.5], rot_4d: bool = False, force_sh_3d: bool = False, sh_degree_t : int = 0, rgbfunction="rgbv1"):
    ...
    def capture(self):
        if self.gaussian_dim == 3:
            ...
            return (
                ...
                self._spacetime_mask,
                self._features_t
            )
    
    def restore(self, model_args, training_args, search=False, cameras_extent=0.0, init_mixprob=0.9, fourd_ckpt_path=None):
        if self.gaussian_dim == 3:
            (self.active_sh_degree, 
            self._xyz, 
            self._features_dc, 
        ...
        elif self.gaussian_dim == 4 and search:
            (self.active_sh_degree, 
            self._xyz, 
            self._features_dc, 
            self._features_rest,
            ...
            self.create_weights(init_mixprob)

        elif self.gaussian_dim == 4 and not search:
            (self.active_sh_degree, 
            self._xyz, 
            self._features_dc, 
            self._features_rest,
            ...
            self.active_sh_degree_t,
            self._spacetime_mask,
            self._features_t) = model_args
            

        if training_args is not None:
            self.training_setup(training_args, cameras_extent)
            self.denom = denom


    def create_weights(self, init_mixprob):
