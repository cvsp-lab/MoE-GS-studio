...
from thirdparty.Poly_SHS.scene.dataset_readers import sceneLoadTypeCallbacks
from thirdparty.Poly_SHS.scene.oursfull import SGaussianModel
from arguments import ModelParams
from PIL import Image 
from thirdparty.Poly_SHS.utils.camera_utils import camera_to_JSON, cameraList_from_camInfosv2, cameraList_from_camInfosv2nogt
from thirdparty.Poly_SHS.helper_train import recordpointshelper, getfisheyemapper
import torch 
class SScene:
    ...
    def __init__(self, args : ModelParams, gaussians, load_iteration=None, shuffle=True, resolution_scales=[1.0], multiview=False,duration=150.0, loader="colmap", render=False, save_path=None, cameras_extent=0.0, stg_path=None, init_mixprob=0.9):
        ...
        if render:
            # self.loaded_iter = searchForMaxIteration(os.path.join(self.model_path, "STG", "point_cloud"))
            self.model_path = save_path
            print("Loading trained router in {} at iteration {}".format(self.model_path, self.loaded_iter))
    ...
    def save(self, iteration, save_path):
        if save_path != None:
            self.model_path = save_path
            print("save on " + self.model_path)
        point_cloud_path = os.path.join(self.model_path, "STG", "point_cloud", "iteration_{}".format(iteration))
