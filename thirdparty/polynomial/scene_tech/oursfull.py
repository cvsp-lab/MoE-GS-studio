...
import torch
import numpy as np
from thirdparty.Poly_SHS.utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation
...
from thirdparty.Poly_SHS.utils.graphics_utils import BasicPointCloud
from thirdparty.Poly_SHS.utils.general_utils import strip_symmetric, build_scaling_rotation, update_quaternion
from thirdparty.Poly_SHS.helper_model import getcolormodel, interpolate_point, interpolate_partuse, interpolate_pointv3
...
class SGaussianModel:
    ...
    def construct_list_of_attributes(self):
        ...
        # for i in range(self._features_d.shape[1]):
        #     l.append('f_d_{}'.format(i))
    ...
    def save_ply(self, path):
        ...
        f_t = self._features_t.detach().cpu().numpy()
        # f_d = self._features_d.detach().cpu().numpy()

        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes()]

        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, trbf_center, trbf_scale, normals, motion, f_dc, f_rest, opacities, scale, rotation, omega, f_t), axis=1)
        ...
        ## search mask parmeter
        spacetime_mask = self._spacetime_mask.detach().cpu().numpy()
        torch.save(spacetime_mask, path.replace('point_cloud.ply', 'spacetime_mask.pth') )
    ...
    def load_ply(self, path, cameras_extent, init_mixprob, search=False):
        ...
        if search:
            print("Initialize 3D Weights")
            init_weight = init_mixprob * torch.ones((self._xyz.shape[0],1), dtype=torch.float, device="cuda")
            # init_weight = inverse_sigmoid(init_mixprob * torch.ones((self._xyz.shape[0], 1), dtype=torch.float, device="cuda"))
            ...
            print("Load Search Mask")
            spacetime_mask = torch.load(path.replace('point_cloud.ply', 'spacetime_mask.pth'))
            # self._spacetime_mask = torch.tensor(spacetime_mask, dtype=torch.float, device="cuda")
