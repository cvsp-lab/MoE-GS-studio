...
import torch
import numpy as np
from thirdparty.polynomial.utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation
...
from thirdparty.polynomial.utils.graphics_utils import BasicPointCloud
from thirdparty.polynomial.utils.general_utils import strip_symmetric, build_scaling_rotation, update_quaternion
from thirdparty.polynomial.helper_model import interpolate_point, interpolate_partuse, interpolate_pointv3
from torch.optim.lr_scheduler import ExponentialLR
import numpy as np
from helper_model import getcolormodel

class SGaussianModel:
    ...
    def __init__(self, sh_degree : int, rgbfunction="rgbv1"):
        ...
        self.duration = 1 # prevent zero div
    ...
    def get_motion_mask(self, deltat):
        return torch.cat((self._spacetime_mask, deltat * self._features_t), dim=1)

    def get_mask_td(self, deltat):
        return torch.cat((self._features_d, deltat * self._features_t), dim=1)
    ...
    # @property
    # def get_opacity_with_weight(self):
    #     opacity = self.opacity_activation(self._opacity)
    #     opacity = opacity.detach()
    #     spacetime_mask  = self.opacity_activation(self._spacetime_mask).detach()
    #     opacity = opacity*spacetime_mask.unsqueeze(1)
    ...
    @property
    def get_mask(self):
        return self._spacetime_mask

    def get_mask_at_t(self, t):
        return torch.sigmoid(self.get_mask[:, t])
    ...
    def cache_gradient(self):
        ...
        self._features_rest_grd += self._features_rest.grad.clone() # self._features_t_grd
    ...
    def training_setup(self, training_args):
        ...
        self.weight_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        ...
        ## Spacetime optimizer setting
        # self.mask_optimzier = torch.optim.Adam([self._spacetime_mask],lr = 0.5)
        # self.spacetime_mask_scheduler = ExponentialLR(self.mask_optimzier, gamma=0.97)
        # self.spacetime_mask_args = get_expon_lr_func(lr_init=mask_lr,
        #                                             lr_final=training_args.position_lr_final*self.spatial_lr_scale,
        #                                             lr_delay_mult=training_args.position_lr_delay_mult,
        #                                             max_steps=training_args.position_lr_max_steps)
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "xyz":
                ...
                param_group['lr'] = lr
            elif param_group["name"] == "f_d":
                lr = self.feature_scheduler_args(iteration)
                param_group['lr'] = lr

        return lr

    # def update_mask_learning_rate(self, iteration):
    #     for param_group in self.mask_optimzier.param_groups:
    #         lr = self.spacetime_mask_args(iteration)
    #         param_group['lr'] = lr
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate((xyz, trbf_center, trbf_scale, normals, motion, f_dc, f_rest, opacities, scale, rotation, omega, f_t, spacetime_mask), axis=1)
    ...
    def load_plyandminmaxall(self, path,  maxx, maxy, maxz,  minx, miny, minz):
        ...
        features_rest= torch.cat((self._features_rest, torch.tensor(features_extra[mask], dtype=torch.float, device="cuda")))
        self._features_rest = nn.Parameter(features_dc.requires_grad_(True))
    ...
    def load_ply(self, path, cameras_extent, init_mixprob,search=False):
    ...
    def weight_pruning(self, mask):
        vaild_mask = ~mask

        self._xyz = self._xyz[vaild_mask]
        self._features_dc = self._features_dc[vaild_mask]
        self._features_rest = self._features_rest[vaild_mask]
        self._opacity = self._opacity[vaild_mask]
        self._scaling = self._scaling[vaild_mask]
        self._rotation = self._rotation[vaild_mask]
        self._trbf_center = self._trbf_center[vaild_mask]
        self._trbf_scale = self._trbf_scale[vaild_mask]
        self._motion = self._motion[vaild_mask]
        self._omega = self._omega[vaild_mask]
        self._spacetime_mask = self._spacetime_mask[vaild_mask]
        self._features_t = self._features_t[vaild_mask]

    
    def prune_points_from_3DWeight(self, threshold):
        sigmoid = torch.sigmoid
        spacetime_prob = sigmoid(self._spacetime_mask)
        mask = spacetime_prob >= threshold

        '''
        # mask가 1차원 텐서라고 가정하고 NumPy 배열로 변환
        mask_np = spacetime_prob.detach().cpu().numpy()
        plt.hist(mask_np, bins=50, edgecolor='black')
        plt.xlabel('Probability Values')
        plt.ylabel('3D Weight probs')
        plt.title('Histogram of Spacetime 3D weights')

        plt.savefig('histogram_Spacetime.png')
        plt.close() 
        '''
        ...
        self._trbf_center = self._trbf_center[mask]
        self._trbf_scale = self._trbf_scale[mask]
        self._motion = self._motion[mask]
        self._omega = self._omega[mask]
        self._spacetime_mask = self._spacetime_mask[mask]
        self._features_t = self._features_t[mask]
        self._features_d = self._features_d[mask]
    ...
    def prune_points(self, mask, reset_mode=False):
        ...
        self._features_t = optimizable_tensors["f_t"]
        self._spacetime_mask = optimizable_tensors["f_m"]
        ...
        if reset_mode:
            ...
            self.xyz_gradient_accum = self.xyz_gradient_accum[valid_points_mask]
            self.denom = self.denom[valid_points_mask]
            self.weight_gradient_accum = self.weight_gradient_accum[valid_points_mask]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_trbf_center, new_trbfscale, new_motion, new_omega):
        d = {"xyz": new_xyz,
    ...
    def addgaussians(self, baduvidx, viewpoint_cam, depthmap, gt_image, numperay=3, ratioend=2, trbfcenter=0.5,depthmax=None,shuffle=False):
        ...
        fused_color = RGB2SH(torch.tensor(rgbs))
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda()
        features[:, :3, 0 ] = fused_color
        features[:, 3:, 1:] = 0.0 # why?

        featuredc = features[:,:,0:1].transpose(1, 2).contiguous()
        featurerest = features[:,:,1:].transpose(1, 2).contiguous()
        ...
        for zscale in ratiaolist :
            ...
            new_features_rest.append(featurerest.cuda(0)[selectedmask])
    ...
    def add_weight_grad(self, weight_grad, visible_update_filter):
        self.weight_gradient_accum[visible_update_filter] += torch.norm(weight_grad[visible_update_filter], dim=-1, keepdim=True)
        self.denom[visible_update_filter] += 1
