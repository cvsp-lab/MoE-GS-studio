...
from thirdparty.Deform.scene.hyper_loader import Load_hyper_data, format_hyper_data
import copy
from thirdparty.Deform.utils.graphics_utils import getWorld2View2, focal2fov
import numpy as np
import json
...
def readColmapCamerasDynerf(cam_extrinsics, cam_intrinsics, images_folder, near, far, startime=0, duration=300):
    ...
    for idx, key in enumerate(cam_extrinsics): 
        ...
        for j in range(startime, startime+int(duration)):
            ...
            image_path = os.path.join(images_folder,f"images/{extr.name[:-4]}", "%d.png" % j)
