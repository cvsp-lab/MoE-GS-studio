...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, stage="fine", cam_type=None, basicfunction=None,
    ...
    if "coarse" in stage:
        ...
        if pc.MoE_mean:
            means_stack    = torch.stack([deform_means3D, emb_means3D], dim=1)
            means3D_final = torch.sum(means_stack * pos_gu, dim=1)
        else:
            means3D_final = deform_means3D

        if pc.MoE_rotation:
            rotation_stack = torch.stack([deform_rotations, emb_rotations], dim=1)
...
def render_experts_all(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, stage="fine", cam_type=None, basicfunction=None,
    cam_no=None, frame_no=None, iter=None, num_down_emb_c=5, num_down_emb_f=5, gate_viz=False, random_gate=False):
