...
class Neural3D_NDC_Dataset(Dataset):
    ...
    def load_meta(self):
        ...
        self.image_paths, self.image_poses, self.image_times, N_cam, N_time , cam_nos, frame_nos = self.load_images_path(videos, self.split)
        self.cam_number = N_cam
        self.time_number = N_time
        self.cam_nos = cam_nos
    ...
    def load_images_path(self,videos,split):
        ...
        for index, video_path in enumerate(videos):
            ...
            video_images_path = video_path.split('.')[0]
            image_path = os.path.join(video_images_path,"images")
            cam_name = video_images_path.split('/')[-1]
            image_path = os.path.join(self.root_dir, "images", cam_name)
            video_frames = cv2.VideoCapture(video_path)
            if not os.path.exists(image_path):
            ...
            for idx, path in enumerate(images_path):
                ...
                cam_no = int(image_path.split('/')[-1][-2:])
                cam_nos.append(cam_no)
                frame_no = int(path[:-4])
            ...
            N_time = len(images_path)
                #     video_data_save[count] = img.permute(1,2,0)
                #     count += 1
        return image_paths, image_poses, image_times, N_cams, N_time, cam_nos, frame_nos
    def __len__(self):
    ...
    def __getitem__(self,index):
        ...
        return img, self.image_poses[index], self.image_times[index], self.cam_nos[index], self.frame_nos[index]
