...
def getcolmapsinglen3d(folder):
   ...
   # threshold is from   https://github.com/google-research/multinerf/blob/5b4d4f64608ec8077222c52fdf814d40acc10bc1/scripts/local_colmap_and_resize.sh#L62
    ...
    patch_match_stereo_cmd = f"colmap patch_match_stereo --workspace_path {workspace_path} --PatchMatchStereo.gpu_index 0"
    exit_code = os.system(patch_match_stereo_cmd)
    if exit_code != 0:
        exit(exit_code)
    print(patch_match_stereo_cmd)

    stereo_fusion_cmd = f"colmap stereo_fusion --workspace_path {workspace_path} --output_path {os.path.join(workspace_path, 'fused.ply')}"
...
def getcolmapsingletechni(folder):
...
def getcolmapsinglehyper(folder):
    folder = os.path.join(folder, "colmap")
    assert os.path.exists(folder)
    ...
    manualinputfolder = os.path.join(folder, "manual")
    image_path = os.path.join(manualinputfolder, "images.txt")
    camera_path = os.path.join(manualinputfolder, "cameras.txt")
    if not os.path.exists(distortedmodel):
        os.makedirs(distortedmodel)

    featureextract = "colmap feature_extractor --database_path " + dbfile+ " --image_path " + inputimagefolder + " --SiftExtraction.max_image_size 4096 --SiftExtraction.max_num_features 16384 --SiftExtraction.estimate_affine_shape 1 --SiftExtraction.domain_size_pooling 1"
    exit_code = os.system(featureextract)
    if exit_code != 0:
        exit(exit_code)
